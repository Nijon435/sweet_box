...
function getCategories() {
  ...
  (appState.inventory || []).forEach((item) => {
    if (item.category && item.category !== "Supplies") {
      ...
      if (normalizedCategory.includes('cake') || normalizedCategory.includes('pastries')) {
...
function completeOrder() {
  ...
  if (posCart.length === 0) {
    showStyledAlert("Cart is empty", "Please add items to the cart before completing the order.", "warning");
    return;
  }
  ...
  if (!customer) {
    showStyledAlert("Customer Required", "Please enter a customer name or table number before completing the order.", "warning");
    if (customerInput) {
      customerInput.focus();
...
function showStyledAlert(title, message, type = "info") {
  ...
  modal.style.cssText = "position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); display: flex; align-items: center; justify-content: center; z-index: 9999; backdrop-filter: blur(4px); animation: fadeIn 0.2s ease-out;";
...
function showCompleteOrderModal(customer, orderType) {
  const modal = document.createElement("div");
  modal.style.cssText = "position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.6); display: flex; align-items: center; justify-content: center; z-index: 9999; backdrop-filter: blur(4px); animation: fadeIn 0.2s ease-out;";

  const total = posCart.reduce((sum, item) => sum + item.qty * item.unitPrice, 0);
  const itemCount = posCart.reduce((sum, item) => sum + item.qty, 0);
  const itemsList = posCart.map(item => `<div style="display: flex; justify-content: space-between; padding: 0.5rem 0; border-bottom: 1px solid #f3f4f6;"><span>${item.qty}x ${item.name}</span><span style="font-weight: 600;">₱${(item.qty * item.unitPrice).toFixed(2)}</span></div>`).join('');
  ...
  modal.innerHTML = `
    <div style="background: white; border-radius: 16px; padding: 2rem; max-width: 500px; width: 90%; box-shadow: 0 20px 60px rgba(0,0,0,0.3); animation: slideUp 0.3s ease-out;">
      ...
      <div style="background: #f9fafb; border-radius: 12px; padding: 1.25rem; margin-bottom: 1.5rem;">
        ...
        <div style="font-size: 0.9rem; color: #6b7280; margin-bottom: 0.5rem;">${itemCount} item${itemCount !== 1 ? 's' : ''}</div>
        ...
        <div style="display: flex; justify-content: space-between; margin-top: 1rem; padding-top: 1rem; border-top: 2px solid #e5e7eb;">
          ...
          <span style="font-size: 1.3rem; font-weight: 700; color: #8b5cf6;">₱${total.toFixed(2)}</span>
        </div>
      </div>
...
function processOrder(customer, orderType) {
  ...
  showStyledAlert("Order Complete", `Order ${order.id} created successfully for ${customer}!`, "success");
...
function showClearCartModal() {
  ...
  modal.innerHTML = `
    <div style="background: white; border-radius: 16px; padding: 2rem; max-width: 420px; width: 90%; box-shadow: 0 20px 60px rgba(0,0,0,0.3); animation: slideUp 0.3s ease-out;">
      <div style="text-align: center; margin-bottom: 1.5rem;">
        <div style="width: 70px; height: 70px; background: linear-gradient(135deg, #ef4444 0%, #dc2626 100%); border-radius: 50%; margin: 0 auto 1rem; display: flex; align-items: center; justify-content: center; box-shadow: 0 8px 20px rgba(239, 68, 68, 0.4);">
          <span style="color: white; font-size: 2.5rem;">🗑️</span>
        </div>
        <h3 style="margin: 0 0 0.75rem 0; font-size: 1.5rem; color: #1f2937;">Clear Cart?</h3>
        <p style="margin: 0; color: #6b7280; font-size: 1rem; line-height: 1.5;">This will remove all ${itemCount} item${itemCount !== 1 ? 's' : ''} from your cart. This action cannot be undone.</p>
      </div>
      <div style="display: flex; gap: 0.75rem;">
        <button onclick="this.closest('[style*=\\'position: fixed\\']').remove()" style="flex: 1; padding: 0.875rem; border: 2px solid #e5e7eb; background: white; border-radius: 10px; cursor: pointer; font-weight: 600; color: #6b7280; transition: all 0.2s;" onmouseover="this.style.background='#f9fafb'" onmouseout="this.style.background='white'">Cancel</button>
  ...
  document.getElementById('confirm-clear-cart').addEventListener('click', () => {
    ...
    renderCart();
    modal.remove();
    showStyledAlert("Cart Cleared", "All items have been removed from the cart.", "info");
  });
}
...
function updateOrderStatistics() {
  ...
  if (!totalOrdersEl || !todayOrdersEl || !totalRevenueEl) return;
